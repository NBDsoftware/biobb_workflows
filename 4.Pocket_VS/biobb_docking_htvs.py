...
# STEP 1: Pocket selection from filtered list 
    ...
    # If model and pocket are provided through arguments -> prioritize over input.yml
...
# STEP 4-5-6-7: For each ligand in library: obtain molecule, prepare ligand, run docking, prepare poses
    ...
    if (ligand_lib_path):
        ...
        ligand_list = readLigandLibFile(ligand_lib_path)
    else:
...
# STEP 8: Find top ligands (according to lowest affinity)
    ...
    bestAffinities, bestLigands = findTopLigands(paths, props, ligand_list, global_log)
