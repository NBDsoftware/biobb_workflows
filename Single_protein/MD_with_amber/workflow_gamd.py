...
def main(configuration_path, output_path = None, input_pdb_path = None):
    ...
    global_log.info("step18_pmemd_mdrun_gamd: Collecting statistics")
    pmemd_mdrun(**global_paths["step18_pmemd_mdrun_gamd"], properties=global_prop["step18_pmemd_mdrun_gamd"])

    global_log.info("step19_pmemd_mdrun_gamd: Creating portable binary run file to run a GaMD simulation")
    pmemd_mdrun(**global_paths["step19_pmemd_mdrun_gamd"], properties=global_prop["step19_pmemd_mdrun_gamd"])
    ...
    global_log.info('  Config File: %s' % conf)
    if system:
