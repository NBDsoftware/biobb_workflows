...
def rmsd_clustering(input_zip_path: str, output_zip_path: str, properties: dict, ranking_df: pd.DataFrame):
    ...
    rmsd_matrix = compute_rmsd_matrix(poses_ranked_paths, ligand_chain)
...
def compute_rmsd_matrix(poses_ranked_paths: list, ligand_chain: str):
    ...
    # Create a shared memory array to store the RMSD matrix
    shared_rmsd_matrix = mp.Array('f', n_poses * n_poses)

    # Get the RMSD matrix from the shared memory array
    rmsd_matrix = np.frombuffer(shared_rmsd_matrix.get_obj(), dtype=np.float32).reshape((n_poses, n_poses))
    ...
    # Find the number of cores to use for multiprocessing
    ...
    if num_threads > 1:
        ...
        # Constant arguments of the compute_rmsd function
        constant_args = (poses_ranked_paths, ligand_chain, ca_serials)

        # Create a partial function to compute RMSD between two poses
        partial_compute_rmsd = partial(compute_rmsd, *constant_args)

        # Parallel
        # Create a pool of worker processes
        pool = mp.Pool(processes=num_threads)
        
        # Create a list with all pairs of poses
        poses_pairs = [(i, j) for i in range(n_poses) for j in range(i + 1, n_poses)]

        # Use multiprocessing to compute RMSD for all pairs of poses
        results = pool.starmap(partial_compute_rmsd, poses_pairs)

        # Close the pool
        pool.close()
        pool.join()

        # Fill the RMSD matrix
        for (i, j), rmsd in zip(poses_pairs, results):
        ...
        for i in range(n_poses):
            for j in range(i + 1, n_poses):
                rmsd_matrix[i, j] = compute_rmsd(poses_ranked_paths, ligand_chain, ca_serials, i, j)
                rmsd_matrix[j, i] = rmsd_matrix[i, j]

    return rmsd_matrix

def compute_rmsd(poses_ranked_paths: list, ligand_chain: str, ca_serials: list, i: int, j: int):
    """
    Compute the RMSD between a reference pose and a target pose using only the CA atoms of the ligand (as the receptor didn't move during docking).
    Saves the RMSD value in the RMSD matrix.
    ...
    ------
        ...
        poses_ranked_paths (list): list with paths to docking poses
        ligand_chain        (str): chain of the ligand protein
        ca_serials         (list): list with the serial numbers of the sampled CA atoms
        i                   (int): index of the reference pose
        j                   (int): index of the target pose
    
    Returns
    -------

        rmsd (float): RMSD between the reference and target poses
    """ 

    # Get the paths to the reference and target poses
    reference_pdb_path = poses_ranked_paths[i]
    target_pdb_path = poses_ranked_paths[j]

    # Find coordinates the CA atoms of the ligand chain for reference pose
    ref_coords = get_ref_coords(reference_pdb_path, ligand_chain, ca_serials)
...
def get_ref_coords(pdb_path: str, ligand_chain: str, ca_serials: list):
    """
    ...
    -------
        ...
        ref_coords (np.array): array with the coordinates of the selected CA atoms of the reference pose
    """
    # Create a PDB parser
    parser = PDB.PDBParser(QUIET=True)
...
def plot_dendogram(linkage_matrix: np.array, labels: list, rmsd_threshold: float, output_path: str):
    """ 
    ...
    ------
            ...
            linkage_matrix (np.array): linkage matrix
            labels             (list): list with labels for each pose
            rmsd_threshold    (float): RMSD threshold to color the dendogram
            output_path         (str): path to output folder
    """
