...
def analyzeDockingResults(wdir, modelName, docking_output_dir, bestAffinities, global_log, avg_affinity_history, pocket_ID = None):
    '''
...
def pocketsVirtualScreening(configuration_path, input_structures_path, ligand_lib_path, pocket_residues_list, 
    ...
    for model_index in range(len(all_models)):
        ...
        if pocket_residues_list:
            ...
            analyzeDockingResults(wdir, modelName, docking_output_dir, bestAffinities, global_log, avg_affinity_history)
...
# STEP 3: Use active ligands to screen the filtered cavities or pocket defined by residues
    ...
    shutil.move(cavity_output_dir, new_output_dir)
